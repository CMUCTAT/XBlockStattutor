...
a[d]&&(a[d]=c[d])}return a}function g(a,b){return"string"==typeof a?a:a[b%a.length]}function h(a){this.opts=c(a||{},h.defaults,l)}function m(){function c(b,c){return a("<"+b+' xmlns="urn:schemas-microsoft.com:vml" class="spin-vml">',c)}x.addRule(".spin-vml","behavior:url(#default#VML)");h.prototype.lines=function(a,e){function h(){return d(c("group",{coordsize:f+" "+f,coordorigin:-r+" "+-r}),{width:f,height:f})}function m(a,m,f){b(v,b(d(h(),{rotation:360/e.lines*a+"deg",left:~~m}),b(d(c("roundrect",
{arcsize:e.corners}),{width:r,height:e.scale*e.width,left:e.scale*e.radius,top:-e.scale*e.width>>1,filter:f}),c("fill",{color:g(e.color,a),opacity:e.opacity}),c("stroke",{opacity:0}))))}var r=e.scale*(e.length+e.width),f=2*e.scale*r,l=-(e.width+e.length)*e.scale*2+"px",v=d(h(),{position:"absolute",top:l,left:l});if(e.shadow)for(l=1;l<=e.lines;l++)m(l,-2,"progid:DXImageTransform.Microsoft.Blur(pixelradius=2,makeshadow=1,shadowopacity=.3)");for(l=1;l<=e.lines;l++)m(l);return b(a,v)};h.prototype.opacity=
...
{get:function(){if(!e.hasOwnProperty("max_loc")){var a=this.end.clone(),b=CTAT.Geom.Point.polar(20,this.angle()),a=a.add(b.scale(-1));a.x=CTAT.Math.round(a.x);a.y=CTAT.Math.round(a.y);e.max_loc=a}return e.max_loc}});var h=c;Object.defineProperty(this,"min",{get:function(){return h},set:function(a){e={};h=a}});var l=d;Object.defineProperty(this,"max",{get:function(){return l},set:function(a){e={};l=a}});Object.defineProperty(this,"length",{get:function(){e.hasOwnProperty("length")||(e.length=CTAT.Math.round(this.min_loc.distance(this.max_loc)));
...
var m=2*Math.PI*a.valueOf();l=e.add(CTAT.Geom.Point.polar(d,m));b.reduce();var n=e.add(CTAT.Geom.Point.polar(d,2*Math.PI*b.valueOf()+m)),r=e.clone();if(0<c&&c<d){m=CTAT.Geom.Point.polar(c,m+Math.PI*b.valueOf());r=r.add(m);l=l.add(m);var p=function(a,b,c){return a.distance(b)<a.distance(c)?b:c};l=CTAT.Geom.Point.circle_intersection(r,l,e,d).reduce(p.bind(this,l));n=n.add(m);n=CTAT.Geom.Point.circle_intersection(r,n,e,d).reduce(p.bind(this,n))}m=CTAT.Geom.Point.to2DString;r="M "+m(r);p=new DOMPoint(d,
d);r+=" L "+m(l);r+=" A "+m(p)+" 0 "+(.5<b.valueOf()?1:0)+",1 "+m(n);g.setAttributeNS(null,"d",r+" Z");h.addPieceElem(g,b,b.selected)}return a.add(b)},new CTAT.Math.Fraction);this.baseGroup.innerHTML="";this.baseGroup.appendChild(f);this.setColor(this.color);this.setDeselectedColor(this.deselectedColor);this.setDeselectedColorAlpha(this.deselectedColorAlpha)}};this.getExplode=function(){return c};this.getDropShadowDistance=function(){return g};this.setExplode=function(a){a=Number(a);if(!isNaN(a)){var b=
...
28:h}),b(q,[2,10],{22:9,19:25,16:p,18:x,20:l,21:r,24:d,25:c,27:g,28:h}),b(v,[2,12]),b(v,[2,13]),b(v,[2,15]),b(v,[2,16]),b(v,[2,21]),b(B,[2,24])],defaultActions:{14:[2,1]},parseError:function(a,b){if(b.recoverable)this.trace(a);else{var c=function(a,b){this.message=a;this.hash=b};c.prototype=Error;throw new c(a,b);}},parse:function(a){var b=[0],c=[null],d=[],e=this.table,f="",g=0,l=0,h=0,n=d.slice.call(arguments,1),m=Object.create(this.lexer),p={},r;for(r in this.yy)Object.prototype.hasOwnProperty.call(this.yy,
r)&&(p[r]=this.yy[r]);m.setInput(a,p);p.lexer=m;p.parser=this;"undefined"==typeof m.yylloc&&(m.yylloc={});r=m.yylloc;d.push(r);var q=m.options&&m.options.ranges;this.parseError="function"===typeof p.parseError?p.parseError:Object.getPrototypeOf(this).parseError;for(var v,x,u,B,M={},P,K;;){u=b[b.length-1];if(this.defaultActions[u])B=this.defaultActions[u];else{if(null===v||"undefined"==typeof v)v=m.lex()||1,"number"!==typeof v&&(v=this.symbols_[v]||v);B=e[u]&&e[u][v]}if("undefined"===typeof B||!B.length||
!B[0]){var R;K=[];for(P in e[u])this.terminals_[P]&&2<P&&K.push("'"+this.terminals_[P]+"'");R=m.showPosition?"Parse error on line "+(g+1)+":\n"+m.showPosition()+"\nExpecting "+K.join(", ")+", got '"+(this.terminals_[v]||v)+"'":"Parse error on line "+(g+1)+": Unexpected "+(1==v?"end of input":"'"+(this.terminals_[v]||v)+"'");this.parseError(R,{text:m.match,token:this.terminals_[v]||v,line:m.yylineno,loc:r,expected:K})}if(B[0]instanceof Array&&1<B.length)throw Error("Parse Error: multiple actions possible at state: "+
u+", token: "+v);switch(B[0]){case 1:b.push(v);c.push(m.yytext);d.push(m.yylloc);b.push(B[1]);v=null;x?(v=x,x=null):(l=m.yyleng,f=m.yytext,g=m.yylineno,r=m.yylloc,0<h&&h--);break;case 2:K=this.productions_[B[1]][1];M.$=c[c.length-K];M._$={first_line:d[d.length-(K||1)].first_line,last_line:d[d.length-1].last_line,first_column:d[d.length-(K||1)].first_column,last_column:d[d.length-1].last_column};q&&(M._$.range=[d[d.length-(K||1)].range[0],d[d.length-1].range[1]]);u=this.performAction.apply(M,[f,l,
