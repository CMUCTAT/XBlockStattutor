...
c.el=a(null,{className:g.className});d(e,{position:g.position,width:0,zIndex:g.zIndex,left:g.left,top:g.top});b&&b.insertBefore(e,b.firstChild||null);e.setAttribute("role","progressbar");c.lines(e,c.opts);if(!p){var h=0,l=(g.lines-1)*(1-g.direction)/2,f,m=g.fps,v=m/g.speed,u=(1-g.opacity)/(v*g.trail/100),n=v/g.lines;(function y(){h++;for(var a=0;a<g.lines;a++)f=Math.max(1-(h+(g.lines-a)*n)%v*u,g.opacity),c.opacity(e,a*g.direction+l,f,g);c.timeout=c.el&&setTimeout(y,~~(1E3/m))})()}return c},stop:function(){var a=
...
scrimIsUp);$("#scrim").remove();n.removeComponent();u.removeHTMLElements();waitScrim=scrimIsUp=!1;f="";u.ctatdebug("exit scrimDownForced() scrimIsUp "+scrimIsUp)};this.defaultClickHandler=function(){u.ctatdebug("defaultClickHandler ()");u.scrimDown()};this.setInAuthorTime=function(a){u.ctatdebug("setting inAuthorTime = "+a);authorTimeSet=!0;inAuthorTime=a;if(!a)warnScrim=errorScrim=!1,g&&null.removeChild(m);else if(errorScrim||warnScrim)for(var c in q)b(c)};this.getInAuthorTime=function(){return inAuthorTime}};
CTATScrim.prototype=Object.create(CTATBase.prototype);CTATScrim.prototype.constructor=CTATScrim;CTATGlobals.languageManager=CTATLanguageManager.theSingleton;function manualCTATScrim(){console.log("manualCTATScrim (): noScrim "+noScrim+", useDebuggingBasic "+useDebuggingBasic);CTATScrim.scrim=new CTATScrim;!1===noScrim&&CTATScrim.scrim.waitScrimUp()}window.jQuery?(ctatdebug("(Scrim) CTATTarget ("+("undefined"==typeof CTATTarget?"undefined":CTATTarget)+")"),manualCTATScrim()):alert("CTAT is unable to access JQuery, please make sure the library is referenced in your html file and loaded before CTAT.");var CTAT={ToolTutor:{interfaceMessages:[],tutorMessages:[]}};
...
(this.ctatdebug("messageParser "+d+", messageObj "+e),e.children&&this.ctatdebug(e.children));var c="ungraded",g=!1,h=!1,l=new CTATSkillSet,n="",q="",p="",x="",m="",v,u=null,B=null,t=!1;this.setURL=function(a){v=a};this.getURL=function(){return v};this.assignSAI=function(a){h=!0;u=a};this.setTransactionID=function(a){q=a};this.getTransactionID=function(){return q};this.setGradeResult=function(a){c=a};this.getGradeResult=function(){return c};this.getXMLObject=function(){return e};this.getSkillsObject=
...
case "Skills":l.parseByValue(G);break;case "SuccessMsg":p=a=d.getNodeTextValue(G);break;case "BuggyMsg":x=d.getNodeTextValue(G);break;case "Selection":a=d.getNodeTextValue(G);t.push(G);break;case "Action":c=d.getNodeTextValue(G);D.push(G);break;case "Input":f=d.getNodeTextValue(G);r.push(G);break;case "prompt":q=d.getNodeTextValue(G);break;case "HighlightMsgText":m=d.getNodeTextValue(G);break;case "custom_fields":B=b(G)}}this.ctatdebug("SAI: "+a+","+c+","+f);""!==a&&null!==a?(u=new CTATSAI(a,c,f,
...
CTATSkillSet.skills=new CTATSkillSet),CTATSkillSet.skills.parseByValue(t))}b.processAssociatedRules(h,u,v);a.propagateShellEvent("AssociatedRules",h)}"BuggyMessage"==m&&(e.ctatdebug("Found: BuggyMessage"),b.processBuggyMessage(h),a.propagateShellEvent("BuggyMessage",h));"SuccessMessage"==m&&(e.ctatdebug("Found: SuccessMessage"),b.processSuccessMessage(h),a.propagateShellEvent("SuccessMessage",h));-1!=m.indexOf("InterfaceAction")&&(e.ctatdebug("Found: InterfaceAction"),v=h.getSAI(),-1!=v.getAction().indexOf(":")?
...
a.length+")");for(var b="Unknown",c="Unknown",g=null,h=0;h<a.length;h++){var f=a[h];e.ctatdebug("Inspecting element: "+d.getElementName(f));"WidgetType"==d.getElementName(f)&&(e.ctatdebug("Widget type: "+d.getNodeTextValue(f)),b=d.getNodeTextValue(f));"CommName"==d.getElementName(f)&&(e.ctatdebug("Instance name: "+d.getNodeTextValue(f)),c=d.getNodeTextValue(f));if("serialized"==d.getElementName(f)){e.ctatdebug("Prepareing to de-serialize component ...");for(var m=d.getElementChildren(f),v=0;v<m.length;v++){var u=
m[v];-1!=d.getElementName(u).indexOf("CTAT")&&(g=u)}}"interface"==d.getElementName(f)&&(e.ctatdebug("Storing interface for post start-state reconstruction ..."),CTATGlobals.interfaceElement=f);"script"==d.getElementName(f)&&(e.ctatdebug("Storing and loading main javascript code as defined by the BRD ..."),scriptElement=d.getNodeTextValue(f))}null!==g?this.deserializeComponent(b,c,g):e.ctatdebug("Error: unable to find CTAT serialization point of attachement");e.ctatdebug("processInterfaceDescription () done")};
this.deserializeComponent=function(a,b,c){e.ctatdebug("deserializeComponent ("+a+","+b+")");if(!0===CTATGlobals.ignoreInterfaceDescriptions)e.ctatdebug("ignoreInterfaceDescriptions==true");else if(null===d.getElementAttr(c,"x"))e.ctatdebug("Warning: this component does not have x,y information. Probably an older component");else{var g=d.getElementAttr(c,"x"),h=d.getElementAttr(c,"y"),f=d.getElementAttr(c,"width"),l=d.getElementAttr(c,"height"),n=d.getElementAttr(c,"tabIndex"),u=d.getElementAttr(c,
"zIndex");null===n&&(n=-1);null===u&&(u=0);var B=new CTATComponentDescription;B.type=a;B.name=b;B.x=Math.floor(g);B.y=Math.floor(h);B.tabIndex=n;B.zIndex=u;B.width=Math.floor(f);B.height=Math.floor(l);CTATShellTools.registerComponentDescription(B);c=d.getElementChildren(c);for(g=0;g<c.length;g++){h=c[g];d.getElementName(h);if("Parameters"==d.getElementName(h))for(this.ctatdebug("Processing component parameters ..."),f=d.getElementChildren(h)[0],f=d.getElementChildren(f),l=0;l<f.length;l++){var t=
f[l];if("CTATComponentParameter"==d.getElementName(t)){this.ctatdebug("Processing parameter property (CTATComponentParameter) ...");for(var u=n=void 0,t=d.getElementChildren(t),z=0;z<t.length;z++){var A=t[z];"name"==d.getElementName(A)&&(this.ctatdebug("Found parameter name: "+d.getNodeTextValue(A)),n=d.getNodeTextValue(A));"value"==d.getElementName(A)&&(this.ctatdebug("Found parameter value: "+d.getNodeTextValue(A)),u=d.getNodeTextValue(A).trim())}this.ctatdebug("Parameter name: "+n+", value: "+
u);void 0!==n&&(B.params[n]=u,"group"==n&&(B.groupName=u))}}if("Styles"==d.getElementName(h))for(this.ctatdebug("Processing component styles ..."),h=d.getElementChildren(h)[0],h=d.getElementChildren(h),f=0;f<h.length;f++)if(u=h[f],"CTATStyleProperty"==d.getElementName(u)){this.ctatdebug("Processing style property (CTATStyleProperty) ...");n=l="";u=d.getElementChildren(u);for(t=0;t<u.length;t++)z=u[t],"name"==d.getElementName(z)&&(l=d.getNodeTextValue(z)),"value"==d.getElementName(z)&&(n=d.getNodeTextValue(z).trim());
...
a;t.style.top=this.y+"px";!0===u&&this.render()};this.setWidth=function(a){w.ctatdebug("setWidth ("+a+")");l=a;t?t.style.width=l+"px":w.ctatdebug("Internal error: no div wrapper available to set width")};this.setHeight=function(a){w.ctatdebug("setHeight ("+a+")");n=a;t?t.style.height=n+"px":w.ctatdebug("Internal error: no div wrapper available to set height")};this.setText=function(a){v=a;return this};this.getValue=function(){return I?I.value:""};this.backgrade=!1;this.unHighlightAll=function(){$(".CTAT--hint").each(function(){for(var a=
...
[],v=this;this.generateClassname=function(){return this.getName()+" UnitPart"};this.getPieces=function(){return m};var u=function(a){if(a.hasAttributeNS(null,"data-selected"))return CTATGlobalFunctions.toBoolean(a.getAttributeNS(null,"data-selected"));a.setAttributeNS(null,"data-selected",!0);return!0};this.getPiecesBySelected=function(a){for(var b=this.getPieces(),c=[],d,e=0;e<b.length;e++)d=b[e],u(d)===a&&c.push(d);return c};this.getSelectedPieces=this.getPiecesBySelected.bind(this,!0);this.getDeselectedPieces=
...
m.classList.add("CTATFractionBar--label");m.setAttributeNS(null,"text-anchor","middle");m.setAttributeNS(null,"dominant-baseline","middle");d.appendChild(m);var x=""+String(f.numerator),f=""+String(f.denominator),v=document.createElementNS(c,"tspan"),u=document.createElementNS(c,"tspan");v.appendChild(document.createTextNode(x));u.appendChild(document.createTextNode(f));m.appendChild(v);m.appendChild(u);m=b+p/2;v.setAttributeNS(null,"x",m+"%");v.setAttributeNS(null,"y","60%");v.setAttributeNS(null,
"dy","-1em");u.setAttributeNS(null,"x",m+"%");u.setAttributeNS(null,"y","60%");x.length>f.length?v.style.textDecoration="underline":u.style.textDecoration="overline";b+=p}this.baseGroup.innerHTML="";this.baseGroup.appendChild(d);var B=this.baseGroup.getBBox().width-1;Array.prototype.slice.call(this.baseGroup.childNodes).forEach(function(a){if("rect"==a.nodeName){var b=window.getComputedStyle(a),c=parseFloat(b.getPropertyValue("padding-left")),d=parseFloat(b.getPropertyValue("stroke-width")),b=a.getBBox();
...
c.y);if(0<Math.abs(CTAT.Math.round(p))){var p=new DOMPoint(a.left,(m-e*a.left)/f),q=new DOMPoint(a.right,(m-e*a.right)/f);if(a.contains(p)||a.contains(q))d.distance(p)<d.distance(q)?(b=q,c=p):(b=p,c=q)}0<Math.abs(CTAT.Math.round(u))&&(u=new DOMPoint((m-f*a.top)/e,a.top),e=new DOMPoint((m-f*a.bottom)/e,a.bottom),a.contains(u)||a.contains(e))&&(d.distance(u)<d.distance(e)?(b=e,c=u):(b=u,c=e));if(this.X_Axis.start.x!=b.x||this.X_Axis.start.y!=b.y||this.X_Axis.end.x!=c.x||this.X_Axis.end.y!=c.y)this.X_Axis.start=
...
f.getLevel()+" ...");var x=document.createElement("div");x.classList.add("CTATSkillWindow--skill");var m=document.createElement("div");m.classList.add("CTATSkillWindow--bar");var v=document.createElement("div"),u=f.getLevel();u<d?v.classList.add("CTATSkillWindow--bar--nonmastered"):v.classList.add("CTATSkillWindow--bar--mastery");v.style.width=u>d?"100%":100*u+"%";m.appendChild(v);x.appendChild(m);b.appendChild(x);m=document.createElement("div");m.textContent=f.getDisplayName()||"no-name";m.classList.add("CTATSkillWindow--label");
...
r="";for(n=0;n<h;n++)if(m=e(a[n]),"string"===m)l[l.length]=a[n];else if("array"===m){v=a[n];if(v[2])for(m=c[g],q=0;q<v[2].length;q++){if(!m.hasOwnProperty(v[2][q]))throw Error(b("[sprintf] property '%s' does not exist",v[2][q]));m=m[v[2][q]]}else m=v[1]?c[v[1]]:c[g++];"function"==e(m)&&(m=m());if(f.test(v[8])&&"number"!=e(m)&&isNaN(m))throw new TypeError(b("[sprintf] expecting number but found %s",e(m)));d.test(v[8])&&(D=0<=m);switch(v[8]){case "b":m=m.toString(2);break;case "c":m=String.fromCharCode(m);
break;case "d":m=parseInt(m,10);break;case "e":m=v[7]?m.toExponential(v[7]):m.toExponential();break;case "f":m=v[7]?parseFloat(m).toFixed(v[7]):parseFloat(m);break;case "o":m=m.toString(8);break;case "s":m=(m=String(m))&&v[7]?m.substring(0,v[7]):m;break;case "u":m>>>=0;break;case "x":m=m.toString(16);break;case "X":m=m.toString(16).toUpperCase()}if(!D||d.test(v[8])&&v[3])r=D?"+":"-",m=m.toString().replace(p,"");q=v[4]?"0"==v[4]?"0":v[4].charAt(1):" ";x=v[6]-(r+m).length;x=v[6]?Array(x+1).join(q):
"";l[l.length]=v[5]?r+m+x:0==q?r+x+m:x+r+m}return l.join("")};b.cache={};b.parse=function(a){for(var b,d=[],e=0;a;){if(null!==(b=c.exec(a)))d[d.length]=b[0];else if(null!==(b=g.exec(a)))d[d.length]="%";else if(null!==(b=h.exec(a))){if(b[2]){var e=e|1,f=[],m=b[2],p;if(null!==(p=l.exec(m)))for(f[f.length]=p[1];""!==(m=m.substring(p[0].length));)if(null!==(p=n.exec(m)))f[f.length]=p[1];else if(null!==(p=q.exec(m)))f[f.length]=p[1];else throw new SyntaxError("[sprintf] failed to parse named argument key");
...
a);break;case 26:this.$=new d.CTATConstantNode(Number(a))}},table:[{3:1,4:2,6:3,13:e,14:4,15:f,17:5,19:8,22:9,24:d,25:c,27:g,28:h},{1:[3]},{5:[1,14]},{5:[2,8],7:[1,15],8:[1,16],9:[1,17],10:[1,18],11:[1,19],12:[1,20],13:l,15:n},b(q,[2,11],{22:9,19:25,16:p,18:x,20:m,21:v,24:d,25:c,27:g,28:h}),b(u,[2,17]),{13:e,15:f,17:28,19:8,22:9,24:d,25:c,27:g,28:h},{13:e,15:f,17:29,19:8,22:9,24:d,25:c,27:g,28:h},b(u,[2,20]),b(u,[2,23],{23:[1,30]}),{13:e,15:f,17:31,19:8,22:9,24:d,25:c,27:g,28:h},{6:32,13:e,14:4,15:f,
...
25:c,27:g,28:h},{13:e,15:f,17:41,19:8,22:9,24:d,25:c,27:g,28:h},{13:e,15:f,17:42,19:8,22:9,24:d,25:c,27:g,28:h},b(u,[2,14]),{13:e,15:f,17:43,19:8,22:9,24:d,25:c,27:g,28:h},{13:e,15:f,17:44,19:8,22:9,24:d,25:c,27:g,28:h},b(u,[2,18]),b(u,[2,19]),{13:e,15:f,17:45,19:8,22:9,24:d,25:c,27:g,28:h},b(u,[2,22]),{13:l,15:n,26:[1,46]},{5:[2,2],13:l,15:n},{5:[2,3],13:l,15:n},{5:[2,4],13:l,15:n},{5:[2,5],13:l,15:n},{5:[2,6],13:l,15:n},{5:[2,7],13:l,15:n},b(q,[2,9],{22:9,19:25,16:p,18:x,20:m,21:v,24:d,25:c,27:g,
28:h}),b(q,[2,10],{22:9,19:25,16:p,18:x,20:m,21:v,24:d,25:c,27:g,28:h}),b(u,[2,12]),b(u,[2,13]),b(u,[2,15]),b(u,[2,16]),b(u,[2,21]),b(B,[2,24])],defaultActions:{14:[2,1]},parseError:function(a,b){if(b.recoverable)this.trace(a);else{var c=function(a,b){this.message=a;this.hash=b};c.prototype=Error;throw new c(a,b);}},parse:function(a){var b=[0],c=[null],d=[],e=this.table,f="",g=0,h=0,m=0,n=d.slice.call(arguments,1),l=Object.create(this.lexer),p={},q;for(q in this.yy)Object.prototype.hasOwnProperty.call(this.yy,
q)&&(p[q]=this.yy[q]);l.setInput(a,p);p.lexer=l;p.parser=this;"undefined"==typeof l.yylloc&&(l.yylloc={});q=l.yylloc;d.push(q);var v=l.options&&l.options.ranges;this.parseError="function"===typeof p.parseError?p.parseError:Object.getPrototypeOf(this).parseError;for(var u,x,t,B,M={},P,K;;){t=b[b.length-1];if(this.defaultActions[t])B=this.defaultActions[t];else{if(null===u||"undefined"==typeof u)u=l.lex()||1,"number"!==typeof u&&(u=this.symbols_[u]||u);B=e[t]&&e[t][u]}if("undefined"===typeof B||!B.length||
!B[0]){var R;K=[];for(P in e[t])this.terminals_[P]&&2<P&&K.push("'"+this.terminals_[P]+"'");R=l.showPosition?"Parse error on line "+(g+1)+":\n"+l.showPosition()+"\nExpecting "+K.join(", ")+", got '"+(this.terminals_[u]||u)+"'":"Parse error on line "+(g+1)+": Unexpected "+(1==u?"end of input":"'"+(this.terminals_[u]||u)+"'");this.parseError(R,{text:l.match,token:this.terminals_[u]||u,line:l.yylineno,loc:q,expected:K})}if(B[0]instanceof Array&&1<B.length)throw Error("Parse Error: multiple actions possible at state: "+
t+", token: "+u);switch(B[0]){case 1:b.push(u);c.push(l.yytext);d.push(l.yylloc);b.push(B[1]);u=null;x?(u=x,x=null):(h=l.yyleng,f=l.yytext,g=l.yylineno,q=l.yylloc,0<m&&m--);break;case 2:K=this.productions_[B[1]][1];M.$=c[c.length-K];M._$={first_line:d[d.length-(K||1)].first_line,last_line:d[d.length-1].last_line,first_column:d[d.length-(K||1)].first_column,last_column:d[d.length-1].last_column};v&&(M._$.range=[d[d.length-(K||1)].range[0],d[d.length-1].range[1]]);t=this.performAction.apply(M,[f,h,
...
d.call(arguments,0):[];return a.apply(b,c)}}(CTATFormulaActions[b],CTATFormulaActions))}a.prototype.evaluate=function(a,g,n,q){var h,l,m,v,u,B,t,z,A,E,I,w;try{z="undefined"!==typeof window&&null!==window?window:global;h=function(){var a;a=[];for(v in z)a.push(v);return a}().concat(["selection","action","input"]);A=a.split(f);l=0;for(u=A.length;l<u;l++)for(t=A[l],I=null!=(E=t.match(b))?E:[],m=0,B=I.length;m<B;m++)v=I[m],0>c.call(e,v)&&0>c.call(h,v)&&0>c.call(this.keys,v)&&CTATFormulaFunctions.assign(v,
...
g["delete"](a))});r.ctatdebug("In evaluate -- to call setInterpretations(), if 0 < newInterps.size "+g.size);0<g.size&&(p(g),w=f);for(var h in z)z.hasOwnProperty(h)&&delete z[h];r.ctatdebug("In evaluate -- out of DoUpdate if branch")}else{r.ctatdebug("In evaluate -- in else branch of doUpdate");a.setNumberOfInterpretations(0);for(var m in z)z.hasOwnProperty(m)&&delete z[m];g.forEach(function(a){z[a.getLastMatchedLink().getUniqueID()]=a.getLastMatchedLink()})}c.toString()===CTATExampleTracerLink.BUGGY_ACTION.toString()&&
...
a+", "+b+", "+c+")");I=a;var e=new Set,m=a.getPreloadedLinkMatches();r.ctatdebug("interps # : "+t.length);r.ctatdebug("interps: "+t);t.forEach(function(d){r.ctatdebug("In evaluate -- entered the interpretations loop");if("undefined"===typeof m||null===m)r.ctatdebug("In evaluate -- entered if check for saiLinkMatches"),m=h(a.getStudentSAI(),a.getActor(),b,d.getVariableTable(),a),r.ctatdebug("In evaluate -- exiting if check for saiLinkMatches");else for(var f=0;f<m.length;++f)g(a.getStudentSAI(),!0,
m[f],m[f].getActor(),d.getVariableTable());r.ctatdebug("Link matches # : "+m.length);m.forEach(function(b){r.ctatdebug("In evaluate -- entered the saiLinkMatches loop + "+b);if(!E.relaxOrderConstraints()&&d.getTraversalCount(b)>=b.getMaxTraversals())return r.ctatdebug("In evaluate -- skipped a saiLinkMatches loop \n Skipped due to max traversals"+b.getMaxTraversals()),!1;var f=d.clone();r.ctatdebug("In evaluate size of getPaths "+f.getPaths().size);f.getPaths().forEach(function(c){r.ctatdebug("In evaluate -- looping through the newInterp.getPaths()\n Examining path: "+
c);!1===l(b,f,c,!1,a)&&(r.ctatdebug("In evaluate -- in if condition, checking if isPathOK"),f.getPaths()["delete"](c),r.ctatdebug("In evaluate -- out of if condition, checking if isPathOK\n Tried to delete path."))});r.ctatdebug("In evaluate -- done looping through the newInterp.getPaths(), size "+f.getPaths().size);if(0<f.getPaths().size){r.ctatdebug("In evaluate -- if number of paths is greater than 0");f.addLink(b);r.ctatdebug("In evaluate -- new link added");if(!0===c){r.ctatdebug("In evaluate -- we are in the doUpdate if condition");
...
" set");var p=d(a,b,c,e);if(null!==p)return p;if(0===e.size)return r.ctatdebug("In evaluate -- if condition no new interpretations"),0!==m.length&&!0===m[0].isDone()&&a.setDoneStepFailed(!0),a.setResult(CTATExampleTracerLink.NO_MODEL),r.ctatdebug("In evaluate -- if condition no new interpretations returning false"),!1;p=q(e);r.ctatdebug("In evaluate -- just found bestInterp");var u=null;!0===a.isSolverResult()?(r.ctatdebug("In evaluate -- in if branch of isSolverResult"),u=CTATExampleTracerLink.NO_MODEL.toString().toUpperCase()===
...
D[c.getUniqueID()])}function h(a,b,c,d,e){r.ctatdebug("CTATExampleTracerTracer --\x3e in findSAIMatchingLinks("+a+", "+b+", "+c+")");var f=[];E.getLinks().forEach(function(h){if(!1===c){r.ctatdebug("Calling link.matchesSAI() on link "+h+", sai "+a);var m=h.matchesSAI(a,b,d);r.ctatdebug("Result from link.matchesSAI() on link "+h+": "+m+", typeof "+typeof m);!0===m&&f.push(h);E.hideButRequireSteps()&&g(a,m,h,b,d)}else!0===h.matchesSAIforHint(a,b,e,d)&&f.push(h)});r.ctatdebug("CTATExampleTracerTracer --\x3e out of findSAIMatchingLinks: #matchingLinks "+
...
f.size);if(!0===d){var h=[],l=!0;b.getMatchedLinks().forEach(function(a){!1!==l&&(!1===E.observesOrderingConstraints(h,a,f,e)||!1===m(b,a,f)?(r.ctatdebug("CTATExampleTracerTracer --\x3e in isPathOK returning false1"),l=!1):h.push(a))});if(!1===l)return r.ctatdebug("CTATExampleTracerTracer --\x3e in isPathOK returning false2"),!1}if(a.getType().toString()===CTATExampleTracerLink.BUGGY_ACTION.toString()){if(!1===E.isIncorrectLinkOK(b.getMatchedLinks(),a,f,b))return r.ctatdebug("CTATExampleTracerTracer --\x3e in isPathOK returning false3"),
!1}else{if(!1===E.observesOrderingConstraints(b.getMatchedLinks(),a,f,e))return r.ctatdebug("CTATExampleTracerTracer --\x3e in isPathOK returning false4"),!1;if(!1===m(b,a,f))return r.ctatdebug("CTATExampleTracerTracer --\x3e in isPathOK returning false5"),!1}r.ctatdebug("CTATExampleTracerTracer --\x3e out of isPathOK returning true");return!0}function n(a,b,c){r.ctatdebug("CTATExampleTracerTracer.replaceInput("+a+", "+b+", ...)");if(null===a||"undefined"===typeof a||!1===a.replaceInput())return r.ctatdebug("replaceInput false, returning null"),
...
t.forEach(function(a){y.ctatdebug(a.getNodeID())});y.ctatdebug("Links in the graph -- ids, from, to");z.forEach(function(a){y.ctatdebug(a)})};this.getSmallestContainingGroup=function(a){return C.getLowestLevelGroupOfLink(a)};this.redoLinkDepths=function(){m();y.getLinks().forEach(function(a){a.setDepth(-1)});y.getLinks().forEach(function(a){-1===a.getDepth()&&u(a)})};this.getGroupModel=function(){return C};this.setVisualData=function(a){y.ctatdebug("CTATExampleTracerGraph --\x3e in setVisualData");
...
CTATExampleTracerGraph.TOP_LEVEL);B=new CTATExampleTracerTracer(y,e);y.ctatdebug("CTATExampleTracerGraph --\x3e out of initGraph")})(!a,b)};Object.defineProperty(CTATExampleTracerGraph,"TOP_LEVEL",{enumerable:!1,configurable:!1,writable:!1,value:"Top Level"});CTATExampleTracerGraph.prototype=Object.create(CTATBase.prototype);CTATExampleTracerGraph.prototype.constructor=CTATExampleTracerGraph;"undefined"!==typeof module&&(module.exports=CTATExampleTracerGraph);var CTATGraphParser=function(){function a(a){ctatdebug("processEdge()");for(var d=new CTATExampleTracerLink(null,null,null),e=g.getElementChildren(a),f,h,m,l,n,p=a=-1,w=CTATMsgType.DEFAULT_ACTOR,x=!1,r=0;r<e.length;r++){"sourceID"===g.getElementName(e[r])&&(a=parseInt(g.getNodeTextValue(e[r])),d.setPrevNode(a));"destID"===g.getElementName(e[r])&&(p=parseInt(g.getNodeTextValue(e[r])),d.setNextNode(p));if("rule"===g.getElementName(e[r]))for(var C=g.getElementChildren(e[r]),F=0;F<C.length;F++)"text"===
...
g.getElementChildren(A[w]);break;case "node":m++;var D;D=A[w];ctatdebug("processNode("+D+")");var r=g.getElementChildren(D),C=-1;D="";for(var F=0;F<r.length;F++)switch(g.getElementName(r[F])){case "uniqueID":C=parseInt(g.getNodeTextValue(r[F]));break;case "text":D=g.getNodeTextValue(r[F])}r=new CTATExampleTracerNode(C,new Set);""!==D&&r.setNodeName(D);D=r;c.addNode(D);1==m&&(c.setStartNode(D),c.setStudentStartsHereNode(D));D.getNodeName()==v&&c.setStudentStartsHereNode(D);break;case "edge":c.addLink(a(A[w]));
...
D,CTATExampleTracerSkill.DEFAULT_P_GUESS,CTATExampleTracerSkill.DEFAULT_P_KNOWN,CTATExampleTracerSkill.DEFAULT_P_SLIP,CTATExampleTracerSkill.DEFAULT_P_LEARN,CTATExampleTracerSkill.DEFAULT_HISTORY),null!=C?y.setLabel(C):y.setLabel(D),null!=F?y.setDescription(F):y.setDescription(D+(r?" "+r:"")),x.push(y))}if(I){m=g.getElementAttr(I,"ordered");if(null===m||"undefined"==typeof isordered)m=!t;ctatdebug("parseGraph() value of isOrdered = "+m);ctatdebug("parseGraph() value of isUnordered = "+t);ctatdebug("parseGraph() typeof isOrdered: "+
typeof m);ctatdebug("parseGraph()typeof isUnordered: "+typeof t);v=I;A=m;ctatdebug("processGroup()");n=c.getGroupModel();m=n.createIdToLinkMap();n.setGroupOrdered(n.getTopLevelGroup(),A);ctatdebug("processGroup() name of group: "+n.getGroupName(n.getTopLevelGroup()));ctatdebug("processGroup() group isOrdered?: "+n.isGroupOrdered(n.getTopLevelGroup()));ctatdebug("processGroup() order of group: isOrdered= "+A);ctatdebug("processGroup() order of group: isOrdered typeof= "+typeof A);v=g.getElementChildren(v);
for(A=0;A<v.length;A++)switch(g.getElementName(v[A])){case "group":e(v[A],m);break;case "outOfOrderMessage":n.getTopLevelGroup().setDefaultBuggyMsg(g.getNodeTextValue(v[A]))}}m=f.getSkillBarVector(!1,!0);if(!m||1>m.length)ctatdebug("No existing skills. Add skills from BRD."),f.addGraphSkills(x),m=f.getSkillBarVector();h=(new CTATTutorMessageBuilder).createStateGraphMessage(q,t,p,f.isSourceFlash()?c.exitOnIncorrectDone():f.isFeedbackSuppressed(),z,E,m);c.redoLinkDepths();d=c.getExampleTracer();d.resetTracer();
...
A=null,d&&(c=h(p?p.getNextNode():null,u.findCurrentState(!1,!1),m.lastMessage?m.lastMessage.getSAI():null,a))&&(a=c);else{m.messageTank.addToMessageTank(CTATMsgType.INCORRECT_ACTION,l.createInCorrectActionMessage(f,a.getInterpolatedSAI()),a,null);m.ctatdebug("about to do silent traceForHint("+b+"), selectionArray "+b.getSelectionArray());var r=m.lastSilentHintResult=new CTATExampleTracerEvent(m,b,c);r.setHintRequest(!0);var t=u.traceForHint(r);m.ctatdebug("silent hint result: "+r+"; hintLink "+t);
t||(t=u.getBestNextLink(!1,r),m.ctatdebug("getBestNextLink result: "+r+"; hintLink "+t));e=null;t&&((e=r.getTutorSAI())&&e.getSelection()&&a.setTutorSAI(e),n=""+t.getUniqueID(),a.setStepID(n));r=g(a,r,l);m.ctatdebug("createBuggyMessage() tutorAdvice: "+r.tutorAdvice+";\n  msg "+r.msg);m.updateSkills(CTATExampleTracerSkill.INCORRECT,t?t.getSkillNames():[],n);m.messageTank.addToMessageTank(CTATMsgType.ASSOCIATED_RULES,l.createAssociatedRulesMessageForAction(a.getResult(),e&&e.getSelection()?e:b,q(c),
b,m.getSkillBarVector(!0,c),n,f,r.tutorAdvice),a,null);r.msg&&m.messageTank.addToMessageTank(r.msgType,r.msg,a);A=b;d&&CTATExampleTracerLink.FIREABLE_BUGGY_ACTION==a.getResult()?(c=h(p?p.getNextNode():null,u.findCurrentState(!1,!1),m.lastMessage?m.lastMessage.getSAI():null,a))&&(a=c):a.isDoneStepFailed()&&m.messageTank.flushDelayedFeedback()}m.lastResult=a;c=m.getProblemSummary(a.getReportableInterpretation());m.ctatdebug("finishNewExampleTrace() to flushMessageTank");m.messageTank.flushMessageTank(c,
!0);m.ctatdebug("finishNewExampleTrace() sent result "+a.getResult()+", studentSAI "+b);return a}function x(){H.transition(CTATMsgType.PROBLEM_RESTORE_END);var a=m.getSkillBarVector(),b=[],c=new CTATTutorMessageBuilder;b.push(c.createMessage(CTATMsgType.PROBLEM_RESTORE_END,a?{Skills:a}:null));m.sendBundle(b)}CTATBase.call(this,"CTATExampleTracer","tracer");var m=this,v=null,u,B,t,z=new CTATXML;new XMLSerializer;var A=null,E,I=!0,w=!1,D=null,r=null,C=null,F=new ProblemStateSaver(m);F.setAuthenticityToken(CTATCommLibrary.getAuthenticityToken());
...
(new CTATTutorMessageBuilder).createMessage(CTATMsgType.PROBLEM_RESTORE_END,null);G.push(a);m.startWorking(1)};this.startWorking=function(b){a(b,!1)};this.stopWorking=function(b){a(b,!0)};this.goToState=function(a){u.resetTracer();return u.goToState(a,m)};this.goToStartState=function(){return m.goToState("start")};this.processXML=function(a){m.ctatdebug("processXML ()");if(null!=a){m.ctatdebug("No of children"+z.getElementChildren(a).length);var b=new CTATGraphParser,c=b.parseGraph(a,m);t=c.gr;u=
c.tracer;a=c.ssm;c=c.sgMsg;m.ctatdebug("sendStartStateMessages() nMsgs "+a.length+", outputStatus "+H);for(var d=[],e=new XMLSerializer,f=null,g=0;g<a.length;g++)if("message"==z.getElementName(a[g])){var l=e.serializeToString(a[g]),n=CTATMsgType.getMessageType(l);CTATMsgType.UNTUTORED_ACTION!=n&&CTATMsgType.INTERFACE_ACTION!=n||m.setVariablesFromSAI(a[g],z);CTATMsgType.START_STATE_END==n?(f=d,t.getFeedbackPolicy()!=CTATMsgType.SHOW_ALL_FEEDBACK&&(n=new CTATSAI("hint","SetVisible","false"),n=(new CTATTutoringServiceMessageBuilder).createInterfaceActionMessage("dummyTransactionID",
n),f.push(n)),f=l,d.push((new CTATTutoringServiceMessageBuilder).createInterfaceConfigurationEnd()),d.push((new CTATTutorMessageBuilder).createLockWidgetMsg(b.getLockWidget()))):d.push(l)}d.unshift(c);m.ctatdebug("sendStartStateMessages(): "+a.length+" elts, "+d.length+" strings, "+H);m.sendBundle(d);b=f;a:{a=[];c=t.getStartNode();d=t.getStudentStartsHereNode();if(null!==c&&null!==d&&c.getNodeID()!=d.getNodeID()){a=t.getBestSubpath(c,d);m.ctatdebug("advanceToStudentStartsHere() path "+a);if(!a)break a;
a=a.getSortedLinks();a=m.traceLinks(a);m.ctatdebug("advanceToStudentStartsHere() results "+a)}d=c=null;0<a.length&&(d=a[a.length-1],a=d.getReportableLink(),null!==a&&(c=a.getNextNode()));m.ctatdebug("advanceToStudentStartsHere() to check for TPA "+c+", "+d);h(c,u.findCurrentState(!1,!1),null,d)}a=[];b&&a.push(b);H.canRestore()||a.push((new CTATTutorMessageBuilder).createMessage(CTATMsgType.PROBLEM_RESTORE_END,null));0<a.length&&m.sendBundle(a);L=!0}m.startWorking(0)};this.addGraphSkills=function(a){m.ctatdebug("adding graph skills: "+
