var StatTutor = {
		...
		process_problem_data: function (data) {
			...
			var make_thought_question = function (panel, name, $question_node) {
				...
				if ($qs.length==1) {
					...
					$qs.each(function() {
						var $this = $(this);
						$q.tabs('add', {title:$this.attr('package'), content: $this.text()});
					});
			...
			$problem_data.find('consider_study_design').each(function() { // should only be one
				...
				var design_thought = $consider_study_design.find('thought_question').each(function() {
					...
					make_thought_question('ConsiderStudyDesign','design_thought',$(this));
				});
			});
			...
			var work_plan_tree = 
				[{text: 'Understand the Problem', iconCls: check_lock('understand_the_problem'),
					id: 'UnderstandTheProblem',
					state: 'closed',
					attributes:{level: 0},
			     	children:
			     		[{text: 'Check Data Format', id: 'CheckDataFormat', iconCls: get_icon(true),
			     			attributes:{level: 1}}, // should always be active for downloading data TODO: add check for datasets and check questions
			     		 {text: 'Consider Study Design', id:'ConsiderStudyDesign', iconCls: get_icon(consider_study_design_active),
			...
			$problem_data.find('main_question').each(function(){
				...
				var i = 0;
				rvs.each(function() { // for each relevant variable for each question
				...
				$mq.find('more_formal_analyses').each(function(){
					...
					$mfa.find('thought_question').each(function(){ // instructions? none in problems
				...
				if (more_formal_analyses_enabled) {
					...
					//$('.'+q+'formal_enabled').css('display','block');
					var $FA = $('#FormalAnalysisQ'+qn);
					$FA.panel({'onOpen':function() {
						var i = this.id;
						var r = i.charAt(i.length-1);
						var $this = $(this);
						if ($('#ref_q'+r+'formal_test').text().replace(/\s|-/gm,'').length>0)
							mark_complete('FormalAnalysisQ'+r);
					},
					'onBeforeOpen': FormalAnalysisQn_onBeforeOpen});
				...
				$mq.find('draw_conclusions').each(function() {
					...
					$dc.find('consider_results').each(function () {
						...
						$cr.find('thought_question').each(function () {
							make_thought_question('ConclusionsConsiderQ'+qn,q+'conclusions_thought',$(this));
						});
					});
				});
				work_plan_tree.push(
					     {text: 'Question '+qn, id: 'Question'+qn, iconCls: 'icon-box',
					     	...
					     	children:
					     		[{text: 'Reflect on Question', id: 'ReflectQ'+qn, iconCls: 'icon-box',
					     			state: 'closed', attributes: {level:0},
					     			children: [{text: 'Consider Question '+StatTutor.number_words[qn],
					     				id:'ConsiderQ'+qn, iconCls: 'icon-box', attributes: {level:1}}]},
					     		 {text: 'Analyze Data', id:'AnalyzeQ'+qn, iconCls: 'icon-box',
					     			state: 'closed', attributes: {level:0},
					     			children: [
					     			    {text: 'Plan Analyses', id:'PlanQ'+qn, iconCls: 'icon-box', attributes: {level:1}},
					     			    {text: 'Exploratory Analysis', id:'ExploreQ'+qn, iconCls: 'icon-box', attributes: {level:1},
					     			    	children: [
					     			    	   {text: 'Determine display and measures', id:'MeasuresQ'+qn, iconCls: 'icon-box', attributes: {level:2}},
					     			    	   {text: 'Conduct Analysis', id: 'AnalysisQ'+qn, iconCls: 'icon-box', attributes: {level:2}},
					     			    	   {text: 'Report Results', id: 'ResultsQ'+qn, iconCls: 'icon-box', attributes: {level:2}}
					     			    ]},
					     			    {text: 'More Formal Analyses', id: 'FormalQ'+qn, iconCls: get_icon(more_formal_analyses_enabled), attributes:{level:1,disabled:!more_formal_analyses_enabled},
					     			    	children: [
					     			    	   {text: 'Determine more formal analyses', id:'FormalMeasuresQ'+qn, iconCls: get_icon(more_formal_analyses_enabled), attributes:{level:2, disabled:!more_formal_analyses_enabled}},
					     			    	   {text: 'Conduct Analysis', id: 'FormalAnalysisQ'+qn, iconCls: get_icon(more_formal_analyses_enabled), attributes:{level:2, disabled:!more_formal_analyses_enabled}},
					     			    	   {text: 'Report Results', id: 'FormalResultsQ'+qn, iconCls: get_icon(more_formal_analyses_enabled), attributes:{level: 2, disabled:!more_formal_analyses_enabled}}
					     			    	]}
					     			]},
					     		 {text: 'Draw Conclusions', id: 'ConclusionsQ'+qn, iconCls: 'icon-box',
					     				state: 'closed', attributes: {level:0},
					     			children: [
					     			   {text: 'Consider what results mean', id: 'ConclusionsConsiderQ'+qn, iconCls: 'icon-box', attributes: {level:1}},
					     			   {text: 'Reflect on conclusions', id: 'ConclusionsReflectQ'+qn, iconCls: 'icon-box', attributes: {level:1}}
					     			]}
					     	]});
			});
			...
			work_plan_tree.push(
			     {text: 'Summarize', id: 'Summarize', iconCls: 'icon-box',
			     	state: 'closed', attributes: {level:0},
			     	children: [
			       {text: 'Summarize Findings', id: 'SummarizeFindings', iconCls: 'icon-box', attributes: {level:1}},
			       {text: 'Evaluate Study', id: 'EvaluateStudy', iconCls: get_icon(validate_study_enabled), attributes:{level:1, disabled:!validate_study_enabled}}
			     ]});
			$('#work_plan').tree({
			...
			$problem_data.find('variable').each(function(){
				...
				$('#problem_data_variables').append('<dt style="font-weight:bold;">'
					+name+'</dt><dd>'+$v.find('description').text()+'</dd>');
				// $v.find('type') => categorical|quantitative
			...
			if ($('#work_plan').tree('isLeaf',node.target)) {
				...
				if (node.children) {
					if (node.children.every(function(c) {
						...
						return check=='complete' || check=='disabled';
					}))	return 'complete';
		...
		update_tree_icon: function (node) {
			...
			if (node){
				...
				if (check=='complete' && (!node.iconCls || node.iconCls!=='icon-ok')) {
					//console.log('  updating to ok');
					$('#work_plan').tree('update', {
						target: node.target,
						iconCls: 'icon-ok'
					});
				} else if (check=='partial' && (!node.iconCls || node.iconCls!=='icon-partial')) {
					//console.log('  updating to partial');
					$('#work_plan').tree('update', {
						target: node.target,
						iconCls: 'icon-partial'
					});
				} else if (check===false && (!node.iconCls || node.iconCls!=='icon-box')) {
					//console.log('  updating to box');
					$('#work_plan').tree('update', {
						target: node.target,
						iconCls: 'icon-box'
					});
...
function mark_complete(node_name) {
	//console.log('mark_complete',node_name);
...
function set_classify_variables(input) {
	...
	var q = inputs.shift();
	//var $qv = $('#'+q+'plan_classify_variables');
	//$qv.html('');
	...
	console.log('set_classify_variables',q,show);
...
function AnalysisQn_onBeforeOpen() {
	...
	console.log(package_select,analysis_type);
